...
class MVSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            # Use Depths/ directory for all image scales
            ...
            if i == 0:  # reference view
                ...
                # Depth maps in Depths/ are at 0.25 scale, need to resize based on image_scale
                if self.image_scale != 0.25:
                    # Get the reference image size to match (image is HWC format from read_img)
                    ref_img = imgs[0]  # First image (reference view) - shape is (H, W, C)
                    target_h, target_w = ref_img.shape[0], ref_img.shape[1]  # H, W
                    depth_h, depth_w = depth.shape[0], depth.shape[1]
                    
                    # Calculate zoom factors
                    zoom_h = target_h / depth_h
                    zoom_w = target_w / depth_w
                    
                    # Resize depth map using nearest neighbor to preserve depth values
                    depth = ndimage.zoom(depth, (zoom_h, zoom_w), order=0, mode='nearest')
