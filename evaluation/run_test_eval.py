...
parser = argparse.ArgumentParser(description='Evaluate checkpoint on test set with statistics')
...
def evaluate_test_set(model, dataloader, evidential_method):
    ...
    Same approach as validation in training (test_sample function).
    
    Returns:
        results: dict with aggregated metrics including peak VRAM
    """
    model.eval()
    ...
    # Reset peak memory stats before inference loop
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(tqdm(dataloader, desc="Evaluating", unit="sample")):
            ...
            # Measure inference time (same as validation approach)
            torch.cuda.synchronize() if torch.cuda.is_available() else None
            time_start = time.perf_counter()
            
            # Model forward (same as test_sample in train.py)
            ...
            # Compute depth using disparity_regression (same as validation via loss_der)
            ...
            # Compute metrics (same as validation: test_sample in train.py)
            scalar_outputs = {}
    ...
    # Synchronize before measuring peak memory
    if torch.cuda.is_available():
